...
```{r}
loadings <- eigen_vecs * matrix(sqrt(eigen_vals), nrow = nrow(eigen_vecs), 
                                ncol = ncol(eigen_vecs), byrow = TRUE)

loadings_df <- data.frame(
  Feature = colnames(wine)[-1],
  Loading = loadings[, 1]
)
...
load(here::here("mnist_umap.RData"))
...
load(here::here("mnist_pca.RData"))
